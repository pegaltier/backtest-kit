...
test("Backtest.dump accepts custom columns", async ({ pass, fail }) => {
  const [awaiter, { resolve }] = createAwaiter();
  ...
  const unsubscribe = listenDoneBacktest((event) => {
    if (event.backtest === true && event.strategyName === "test-strategy-custom-dump") {
      resolve(true);
      unsubscribe();
    }
  });
  ...
  try {
    ...
    const reportPath = path.join(process.cwd(), "./test/temp/test-strategy-custom-dump.md");

    if (fs.existsSync(reportPath)) {
      const content = fs.readFileSync(reportPath, "utf-8");

      const hasCustomHeader = content.includes("| Signal |");
      const hasResult = content.includes("| Result |");

      // Clean up
      fs.unlinkSync(reportPath);

      if (hasCustomHeader && hasResult) {
        pass("Backtest.dump accepts custom columns");
        return;
      }
    }
  } catch (error) {
    fail(`Backtest.dump failed: ${error.message}`);
    return;
  }

  fail("Custom columns in dump were not applied correctly");
});
