...
test("Performance.clear clears statistics", async ({ pass, fail }) => {
  const [awaiter, { resolve }] = createAwaiter();

  addExchange({
    exchangeName: "binance-perf-clear",
    getCandles: async (_symbol, interval, since, limit) => {
      return await getMockCandles(interval, since, limit);
    },
    formatPrice: async (symbol, price) => {
  ...
  fail(`Performance data not cleared: ${statsAfter.totalEvents} events remaining`);
});
