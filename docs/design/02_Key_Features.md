...
For information about the overall architecture and layer separation, see [Architecture](04_Architecture.md). For implementation details of specific components, see [Core Business Logic](14_Core_Business_Logic.md) and [Service Layer](18_Service_Layer.md).
...
Every signal generated by `getSignal()` is validated before execution to prevent invalid trades. Validation failures throw descriptive errors.
...
Signals follow a deterministic state machine with discriminated union types for type-safe handling:
...
| State | Entry Point | Exit Point |
|-------|-------------|------------|
| **idle** | [src/client/ClientStrategy.ts:306-322]() | `getSignal()` returns non-null |
| **opened** | [src/client/ClientStrategy.ts:275-291]() | Next tick iteration |
...
All price monitoring uses Volume-Weighted Average Price (VWAP) calculated from the last 5 one-minute candles, providing more accurate price discovery than simple close prices:
...
The framework generates detailed markdown reports with statistics for both backtest and live trading:
...
- Signal-by-signal table with prices, PNL, close reason, duration
...
- Signal-by-signal table with all details

### Report API Methods

| Method | Backtest | Live | Purpose |
|--------|----------|------|---------|
