...
const news_long_range_columns: IColumn[] = [
    {
...
export const LongRangeView = () => {
    const handler = useLongRangeArrayPaginator();

    const { render, pickColumns, columns } = useColumnConfig({
        columns: news_long_range_columns,
        storageKey: "news_long_range_columns",
    });
    ...
    const handleClick = async (row: any) => {
        ...
        ioc.layoutService.pickLongRange(row.id);
    };
