...
import { ioc } from "../../../../lib";
import useShortRangeArrayPaginator from "../../../../api/useShortRangeArrayPaginator";
...
const news_short_range_columns: IColumn[] = [
    {
...
export const ShortRangeView = () => {
    const handler = useShortRangeArrayPaginator();

    const { render, pickColumns, columns } = useColumnConfig({
        columns: news_short_range_columns,
        storageKey: "news_short_range_columns",
    });
    ...
    const handleClick = async (row: any) => {
        ...
        ioc.layoutService.pickShortRange(row.id);
    };
