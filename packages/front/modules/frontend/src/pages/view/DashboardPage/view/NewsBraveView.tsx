...
import { ioc } from "../../../../lib";
import useBraveArrayPaginator from "../../../../api/useBraveArrayPaginator";
...
const news_brave_columns: IColumn[] = [
    {
...
export const NewsBraveView = () => {
    const handler = useBraveArrayPaginator();

    const { render, pickColumns, columns } = useColumnConfig({
        columns: news_brave_columns,
        storageKey: "news_brave_columns",
    });
    ...
    const handleClick = async (row: any) => {
        ...
        ioc.layoutService.pickWebSearch(row.id);
    };
