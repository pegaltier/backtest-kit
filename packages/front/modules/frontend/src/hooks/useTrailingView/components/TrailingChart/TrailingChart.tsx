...
interface IChartProps {
  ...
  currentPrice: number;
  percentShift: number;
  trailingType: "trailing_stop.commit" | "trailing_take.commit";
  createdAt: string;
}
...
export const TrailingChart = ({
  ...
  useLayoutEffect(() => {
    ...
    const lineSeries = chart.addLineSeries({
      ...
      color: TRAILING_PURPLE,
    });

    lineSeries.setData(candles);

    // Current price line at trailing commit moment
    lineSeries.createPriceLine({
      ...
      title: trailingType === "trailing_stop.commit" ? "Trailing SL" : "Trailing TP",
    });

    // Markers for trailing commit point
    const markers: SeriesMarker<Time>[] = [];

    // Trailing commit marker (createdAt)
    ...
    if (commitDate.isValid()) {
      ...
      const shiftLabel = percentShift >= 0 ? `+${percentShift.toFixed(2)}%` : `${percentShift.toFixed(2)}%`;
      const markerLabel = trailingType === "trailing_stop.commit" ? `SL ${shiftLabel}` : `TP ${shiftLabel}`;
  ...
  }, [source, height, width, items, currentPrice, percentShift, trailingType, createdAt]);
