...
const DEFAULT_PATH = "/partial_profit";

const history = createMemoryHistory();

const fetchData = async (id: string) => {

  const partialProfitData = await ioc.notificationViewService.getOne(id) as PartialProfitNotification;

  if (!partialProfitData) {
    throw new Error("Partial profit data not found");
  }

  if (partialProfitData.type !== "partial_profit.available" && partialProfitData.type !== "partial_profit.commit") {
    throw new Error(`Invalid notification data type: expected 'partial_profit.available' or 'partial_profit.commit', got ${partialProfitData.type}`);
  }

  return {
    partialProfit: partialProfitData,
    candle_1m: await ioc.exchangeViewService.getPointCandles({
      currentTime: partialProfitData.timestamp,
      exchangeName: partialProfitData.exchangeName,
      interval: "1m",
      symbol: partialProfitData.symbol,
    }),
...
const handleDownload = async (pathname: string, id: string) => {
  ...
  const { candle_15m, candle_1h, candle_1m, partialProfit } = await fetchData(id);

  if (pathname.includes("/partial_profit")) {
    const blob = new Blob([JSON.stringify(partialProfit, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `partial_profit_${partialProfit.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1m_${partialProfit.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_15m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_15m_${partialProfit.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1h")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1h_${partialProfit.id || "unknown"}.json`);
    return;
  }
};

export const usePartialProfitView = () => {
  ...
  const handleTabChange = (id: string, history: History) => {
    if (id === "partial_profit") {
      history.replace(`/partial_profit`);
      setPathname(`/partial_profit`);
    }
    if (id === "candle_1m") {
  ...
  const { pickData, render } = useTabsModal({
    ...
    mapInitialData: ([{ partialProfit, ...other }]) => ({
      main: partialProfit,
      partialProfit,
      ...other,
    }),
...
export default usePartialProfitView;
