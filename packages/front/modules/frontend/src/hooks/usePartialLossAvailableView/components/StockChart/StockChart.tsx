...
export const StockChart = ({
  ...
  useLayoutEffect(() => {
    ...
    // Calculate and show the loss level price line
    const lossLevelPrice = position === "long"
      ? priceOpen * (1 - level / 100)
      : priceOpen * (1 + level / 100);

    lineSeries.createPriceLine({
      price: lossLevelPrice,
      color: RED_COLOR,
      lineWidth: 2,
      lineStyle: LineStyle.Solid,
      axisLabelVisible: true,
      title: `Loss Level -${level}%`,
    });
  ...
  }, [source, height, width, items, position, priceOpen, currentPrice, level, createdAt]);
