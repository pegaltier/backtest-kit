...
const fetchData = ttl(async (id: string) => {
  ...
  const signalSyncCloseData = await ioc.notificationViewService.getOne(id) as SignalSyncCloseNotification;

  if (!signalSyncCloseData) {
    throw new Error("Signal sync close data not found");
  }

  if (signalSyncCloseData.type !== "signal_sync.close") {
    throw new Error(`Invalid notification data type: expected 'signal_sync.close', got ${signalSyncCloseData.type}`);
  }

  return {
    signal_sync_close: signalSyncCloseData,
    candle_1m: await ioc.exchangeViewService.getPointCandles({
      currentTime: signalSyncCloseData.timestamp,
      exchangeName: signalSyncCloseData.exchangeName,
      interval: "1m",
      symbol: signalSyncCloseData.symbol,
    }),
...
const handleDownloadJson = async (pathname: string, id: string) => {
  ...
  if (pathname.includes("/signal_sync_close")) {
    const blob = new Blob([JSON.stringify(signal_sync_close, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `signal_sync_close_${signal_sync_close.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1m_${signal_sync_close.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_15m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_15m_${signal_sync_close.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1h")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1h_${signal_sync_close.id || "unknown"}.json`);
    return;
  }
};
...
const handleCopy = async (pathname: string, id: string, onCopy: (content: string) => void) => {
  ...
  if (pathname.includes("/signal_sync_close")) {
    onCopy(JSON.stringify(signal_sync_close, null, 2));
    return;
  }
...
const handleDownloadPdf = async (id: string) => {
  const { signal_sync_close } = await fetchData(id);
  if (signal_sync_close) {
    const content = ioc.markdownHelperService.buildMarkdownFromFields(signal_sync_close_fields, signal_sync_close);
...
const handleDownloadMarkdown = async (id: string) => {
  ...
  if (signal_sync_close) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `signal_sync_close_${signal_sync_close.id || "unknown"}.md`);
  }
};

export const useSignalSyncCloseView = () => {
  ...
  const handleTabChange = (id: string, history: History) => {
    if (id === "signal_sync_close") {
      history.replace(`/signal_sync_close`);
      setPathname(`/signal_sync_close`);
    }
    if (id === "candle_1m") {
  ...
  const { pickData, render } = useTabsModal({
    ...
    animation: "none",
    title: "Signal Sync Close Details",
                    ...
                    <ActionIcon
                        ...
                        onClick={() => {
                            ...
                            ioc.routerService.push(
                                `/dump/${signal_sync_close.signalId}`,
                            );
                        }}
                    >
                        <Search />
    ...
    mapInitialData: ([{ signal_sync_close, ...other }]) => ({
      main: signal_sync_close,
      signal_sync_close,
      ...other,
    }),
...
export default useSignalSyncCloseView;
