...
export const StockChart = ({
  ...
  useLayoutEffect(() => {
    ...
    // Markers for cancellation point
    const markers: SeriesMarker<Time>[] = [];

    // Cancellation marker (createdAt)
    const cancelDate = dayjs(createdAt);
    if (cancelDate.isValid()) {
      let cancelTime: Time;
      if (source === "1m") {
        cancelTime = getMomentStamp(cancelDate, "minute") as Time;
      } else if (source === "15m") {
        const minute = Math.floor(cancelDate.minute() / 15) * 15;
        const alignedDate = cancelDate.startOf("hour").add(minute, "minute");
        cancelTime = getMomentStamp(alignedDate, "minute") as Time;
      } else {
        const alignedDate = cancelDate.startOf("hour");
        cancelTime = getMomentStamp(alignedDate, "hour") as Time;
      }
  ...
  }, [source, height, width, items, cancelReason, createdAt]);
