...
const DEFAULT_PATH = "/brave_status/main";

const history = createMemoryHistory();

export const useBraveStatusView = () => {
    const ctx = useModalManager();
    const { push, pop } = ctx;

    const handleDownload = async () => {
        const content = await ioc.revenueViewService.getBraveStatus();
    ...
    const { pickData, render } = useOutletModal({
        ...
        fetchState: async (id) => [
            await ioc.revenueViewService.getBraveStatus(),
            await getPayload(),
        ],
...
export default useBraveStatusView;
