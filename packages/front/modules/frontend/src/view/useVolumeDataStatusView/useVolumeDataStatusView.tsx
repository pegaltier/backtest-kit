...
const DEFAULT_PATH = "/long_range_status/main";

const history = createMemoryHistory();

export const useLongRangeStatusView = () => {
    const ctx = useModalManager();
    const { push, pop } = ctx;

    const handleDownload = async () => {
        const content = await ioc.revenueViewService.getVolumeDataStatus();
    ...
    const { pickData, render } = useOutletModal({
        ...
        animation: "none",
        title: "Индикатор SMA (объем рынка)",
        ...
        fetchState: async (id) => [
            await ioc.revenueViewService.getVolumeDataStatus(),
            await getPayload(),
        ],
...
export default useLongRangeStatusView;
