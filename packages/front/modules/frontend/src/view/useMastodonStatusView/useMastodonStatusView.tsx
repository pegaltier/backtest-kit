...
const DEFAULT_PATH = "/mastodon_status/main";

const history = createMemoryHistory();

export const useMastodonStatusView = () => {
    const ctx = useModalManager();
    const { push, pop } = ctx;

    const handleDownload = async () => {
        const content = await ioc.revenueViewService.getMastodonStatus();
    ...
    const { pickData, render } = useOutletModal({
        ...
        fetchState: async (id) => [
            await ioc.revenueViewService.getMastodonStatus(),
            await getPayload(),
        ],
...
export default useMastodonStatusView;
